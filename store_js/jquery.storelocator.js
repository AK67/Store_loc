...
$.fn.storeLocator = function(options) {
  ...
  var settings = $.extend( {
      ...
      'infowindowTemplatePath': '/static/ss/templates/infowindow-description.html',
      'listTemplatePath': '/static/ss/templates/location-list-description.jsp',
      'KMLinfowindowTemplatePath': '/static/ss/templates/kml-infowindow-description.html',
      'KMLlistTemplatePath': '/static/ss/templates/kml-location-list-description.html',
	  ...
	  'distanceErrorAlert1': '. Please refine your search.',
      'mileLang': 'mile',
      'milesLang': 'miles',
    ...
    this.geocode = function(address, callbackFunction) {
        geocoder.geocode( { 'address': address}, function(results, status) {
          if (status === google.maps.GeocoderStatus.OK) {
		   ...
		   window.strtlat=result.latitude;
            result.longitude = results[0].geometry.location.lng();
			window.strtlng=result.longitude;
  ...
  function begin_mapping(distance){
    var searchstrbread =  $('#proxyaddress').val();
    //show breadcrums on findyour store
  ...
  $(function(){
	...
	settings.distanceAlert = $("#distancelimit").val();
	settings.maxDistance =  $("#distancelimit").val();
      e.preventDefault();
      settings.maxDistance = true;
      if(settings.maxDistance === true){
        var maxDistance = $("#distancelimit").val();
    ...
    if(settings.noForm === true){
      ...
      $(document).on('keyup.'+prefix, function(e){
		...
		$(document).on('submit.'+prefix, '#' + settings.formID, function(e){
			var searchText = $("#proxyaddress").val();
			if(numericFilter(searchText) || stringFilter(searchText)) {
				if($("#proxyaddress").val()!=""){
					$("#address").val($("#proxyaddress").val());
				}		
				settings.dataLocation="/store/account/storefetcherdroplet.jsp?address="+$("#address").val();
				//Locations list
				get_form_values(e);
			} else {
  ...
  $(function(){
        ...
        success: function (data, xhr, options){
            ...
            if(settings.dataType === 'json' || settings.dataType === 'jsonp') {
			  ...
			  var storeCounter = 0;
			  var searchByCity = new Array();
			  var storesExist = false;
			  var pAddress = $("#proxyaddress").val()
              $.each(data, function(){					
                  ...
                  for( key in this ){				  
  				    value = this[key];
					if(key == 'state') {
						//alert("locationData : " + getStateValue(value.toUpperCase()) + " : " + pAddress.toUpperCase());
						if(getStateValue(value.toUpperCase()) === pAddress.toUpperCase() || value.toUpperCase() === pAddress.toUpperCase()) {
							searchedBy = "state";			
						}
					...
					if(searchedBy == 'database') {
						var searchQuery = $("#address").val();
						searchByCity = searchQuery.split(",");
					}
                ...
                if(settings.maxDistance === true && firstRun !== true && maxDistance){
				  ...
				  //alert("state : " + locationData['state']);
                  locationset[i] = locationData;
                }
			  ...
			  if(storesExist == true) {
				//pAddress = $("#proxyaddress").val();
				if(searchedBy != '') {
					loadData(statesArray, cityArray, storeArray, searchedBy, pAddress);
				} else {
					loadStates(statesArray);
				}
			  } else {
				$("#address").val("");
				$("#proxyaddress").val("");
              ...
              $(data).find('marker').each(function(){
                var locationData = {
				  ...
				  'img':$(this).attr('img'),
				  'businessDays':$(this).attr('businessDays'),
				  'storePage':$(this).attr('storePage'),
                };
				
				
				
                      
					 locationData['buisnessDays'] = locationData['buisnessDays'].replace(/,/g , "\n"); 
          ...
          if(settings.maxDistance === true && firstRun !== true && maxDistance){
            if(locationset[0] === undefined  || locationset[0]['distance'] > maxDistance){
              alert(settings.distanceErrorAlert + maxDistance + " " + distUnit + settings.distanceErrorAlert1);
			   $('#maploaderimg').hide();
              return;
            }
          }
             ...
             var key, value, locationData = {};
              ...
              function define_location_data(currentMarker){
                ...
                if(settings.storeLimit === -1 || settings.storeLimit > 26){
				  ...
				  var indicator = String.fromCharCode("A".charCodeAt(0) + markerId);
				  		switch(locationset[markerId]['name']) {
							case "Palais Royal":
								indicator="PR";
								break;
							case "Bealls":
                ...
                var locations = {
                  location: [$.extend(locationData, {
                    ...
                    'length': distLength,
                    'origin': origin,
					'strtlat':window.strtlat,
					'strtlng':window.strtlng
                  })]
                };
              ...
              $this.data('map', map);
				...
				/* var myOptions = {					
					...
					,zIndex: null
					,boxStyle: {
					  //background: "url('http://1.bp.blogspot.com/-wt7SWnCD_Ak/UJA-2rPsoNI/AAAAAAAAALw/fDrmx3Tur40/s400/Black+Stars.jpg') no-repeat"
					   opacity: 0.10
					   ,width: "280px"
					}
					,closeBoxMargin: "0px 2px 2px 2px"
					,closeBoxURL: "http://www.google.com/intl/en_us/mapfiles/close.gif"
					,infowindowClearance: new google.maps.Size(1, 1)
					,isHidden: false
					,pane: "floatPane"
              ...
              if(settings.originMarker === true && settings.fullMapStart === false){
                ...
                noofstores=storenum+1;
				$( "#noOfStoresNearby" ).empty();
                $( "#noOfStoresNearby" ).append( " <p>&nbsp;&nbsp;"+noofstores+" locations near you</p>" );			  
			  
              //Add markers and infowindows loop
              for(var y = 0; y <= storenum; y++) { 
                var letter = String.fromCharCode("A".charCodeAt(0) + y);
				//if(y==0){alert(locationData['name']);}
					switch(locationset[y]['name']) {
							case "Palais Royal":
								letter="PR";
								break;
							case "Bealls":
              ...
              $(document).on('click.'+prefix, '#' + settings.listDiv + ' li .view-onmap' , function(){
                ...
                $('#' + settings.listDiv + ' li[data-markerid=' + markerId +']').addClass('list-focus');
				     ...
				     // alert($('#' + settings.listDiv + ' li[data-markerid=' + markerId +']'));
              ...
              $("#" + settings.listDiv + " ul li:odd").css('background', "#" + settings.listColor1);
               
              //Custom marker function - alphabetical
              function createMarker(point, name, address, letter){
                //Set up pin icon with the Google Charts API for all of our markers
                var pinImage = new google.maps.MarkerImage("http://chart.apis.google.com/chart?chst=d_map_spin&chld=0.75|0|" + settings.pinColor +"|13|b|" + letter);
              ...
              function create_infowindow(marker, location){
				...
				//remove delegate function to go back
               if(location === "left"){
